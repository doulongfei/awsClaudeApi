...
def log_request_info(endpoint, request_data):
    ...
    if DEBUG_MODE:
        # 直接输出完整请求内容
        logger.debug(f"API请求 {endpoint}: {json.dumps(request_data, ensure_ascii=False, indent=2)}")
...
def log_response_info(endpoint, response_data, is_error=False):
    ...
    if DEBUG_MODE:
        if is_error:
            ...
            safe_data = response_data.copy() if isinstance(response_data, dict) else {'data': str(response_data)}
            # 不再截断choices内容，只记录长度信息
            if 'choices' in safe_data and isinstance(safe_data['choices'], list):
                for i, choice in enumerate(safe_data['choices']):
                    if isinstance(choice, dict):
                        if 'text' in choice and isinstance(choice['text'], str):
                            safe_data['choices'][i]['text'] = f"[内容长度: {len(choice['text'])}]"
                        if 'message' in choice and isinstance(choice['message'], dict) and 'content' in choice[
                            'message']:
                            if isinstance(choice['message']['content'], str):
                                safe_data['choices'][i]['message'][
                                    'content'] = f"[内容长度: {len(choice['message']['content'])}]"
                        if 'thinking' in choice and isinstance(choice['thinking'], str):
                            safe_data['choices'][i]['thinking'] = f"[内容长度: {len(choice['thinking'])}]"
...
def validate_bedrock_request(params):
    ...
    if validated_params.get('thinking') and isinstance(validated_params['thinking'], dict):
        ...
        if max_tokens <= thinking_budget:
            logger.warning(
                f"max_tokens({max_tokens})必须大于thinking预算({thinking_budget})，调整max_tokens为{thinking_budget + 1}")
...
def invoke_with_retry(func, **kwargs):
    ...
    if DEBUG_MODE:
        ...
        logger.info(f"请求AWS模型: model_id={model_id}, model_name={model_name if model_name else '未知'}")
                ...
                if 'messages' in safe_body and isinstance(safe_body['messages'], list):
                    for i, msg in enumerate(safe_body['messages']):
                        if 'content' in msg and isinstance(msg['content'], str):
                            safe_body['messages'][i]['content'] = f"[内容长度: {len(msg['content'])}]：{msg['content']}"
...
def completions():
    ...
    try:
        ...
        # 自动根据模型调整最大token限制
        model_max = MODEL_MAX_TOKENS.get(model_name, MAX_OUTPUT_TOKENS)
        if max_tokens > model_max:
            logger.info(
                f"请求的max_tokens({max_tokens})超过了模型{model_name}支持的最大值({model_max})，将使用模型最大值")
...
def chat_completions():
    ...
    try:
        ...
        model_name = req.get('model', 'claude-3-7-sonnet')
        model_id = MODEL_MAPPING.get(model_name, DEFAULT_MODEL_ID)
        logger.info("请求的模型name为：" + model_name)
        logger.info("请求的模型id为：" + model_id)
        ...
        if system_content:
            bed_params['system'] = system_content
        # ====== 修改结束 ======
